...
-- Создание главного окна (вверху будет надпись Tap to minimize)
...
GamesTab:CreateButton({
    ...
    Callback = function()
        ...
        elseif SelectedPlace == "Forsaken" then
            -- Загружает sasaken.lua из твоего репозитория
            loadstring(game:HttpGet(GithubURL .. "sasaken.lua"))()
        elseif SelectedPlace == "Brookhaven" then
            -- Загружает Brookhaven.lua из твоего репозитория
...
elseif currentId == 15312711116 then -- Реальный ID Forsaken
    PlaceDropdown:Set("Forsaken")
elseif currentId == 4924922222 then -- Пример ID Brookhaven
    PlaceDropdown:Set("Brookhaven")
end

Rayfield:Notify({Title = "TRH Visuals", Content = "Successfully loaded via Delta!", Duration = 4})
